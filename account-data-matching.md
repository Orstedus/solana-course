...
- Здійснення перевірки даних на валідність за допомогою обмежень Anchor
---
...
- в Anchor, ви можете використовувати `constraint` щоб перевірити, чи є даний вираз правдивим. Крім того, можна використовувати `has_one` щоб перевірити, чи поле цільового облікового запису зберігається в акаунті, який відповідає ключу у структурі `Accounts`.
...
Anchor облегшує цю задачу за допомогою обмежувача `has_one`. Ви можете використовувати `has_one`, щоб перенести перевірку даних з функції до структури `UpdateAdmin`.
...
Запустіть `anchor test` щоб побачити, що транзакція, яка зроблена за допомогою невірного акаунту зараз повертає помилку Anchor, а та, котра використовує вірний акаунт - виконується без проблем.
...
Зверніть увагу, що Anchor уточнює у консолі той акаунт, який і спричинив помилку (`AnchorError caused by account: vault`).
